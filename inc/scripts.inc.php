...
var url = "https://wwwefiewura.000webhostapp.com";
var done = false;
...
$('#mdad').click(function(){
  ...
  $('input#short_desc').blur(function() {
    var payload = {next:'update',signature:<?php if(isset($space_id))echo "\"".$space_id."\"";else echo "\"\"";?>,short_desc:$('input#short_desc').val()}
    var results;
    $.post(url,payload,function(data, status){results=JSON.parse(data); console.log(results);if (results&&results.status=='success') {/*do something*/}});
  });
  $('input#description').blur(function() {
    var payload = {next:'update',signature:<?php if(isset($space_id))echo "\"".$space_id."\"";else echo "\"\"";?>,description:$('input#description').val()}
    var results;
    $.post(url,payload,function(data, status){results=JSON.parse(data); console.log(results);if (results&&results.status=='success') {/*do something*/}});});
</script>
 <?php
